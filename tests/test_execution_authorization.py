...
Consent binds to the terms it was given for, and to a verdict Guardian actually
issued about this action.
"""
...
def _allow(action: ProposedAction, at: datetime) -> GuardianAssessment:
    return DEFAULT_GUARDIAN_AUTHORITY.allow(
        action_id=action.action_id, fingerprint=action.terms_fingerprint, at=at
    )


def _blocked(action: ProposedAction, at: datetime) -> GuardianAssessment:
    return DEFAULT_GUARDIAN_AUTHORITY.assess(
...
def _policy() -> ExecutionPolicy:
    return ExecutionPolicy(DEFAULT_GUARDIAN_AUTHORITY)
...
def test_guardian_block_defeats_a_valid_authorization(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _blocked(action, now), authorization, now)

    assert decision.outcome is PolicyOutcome.DENY
    assert decision.has_reason(ReasonCode.GUARDIAN_BLOCKED)
    with pytest.raises(AuthorizationRequired):
        require_execution_authorization(action, _blocked(action, now), authorization, now)
...
def test_guardian_escalate_stops_execution_and_asks_for_a_human(
    ...
    guardian = DEFAULT_GUARDIAN_AUTHORITY.assess(
        action_id=action.action_id,
        fingerprint=action.terms_fingerprint,
        at=now,
...
def test_a2_without_authorization_is_not_executable(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, now), None, now)
    ...
    with pytest.raises(AuthorizationRequired, match="AUTHORIZATION_MISSING"):
        require_execution_authorization(action, _allow(action, now), None, now)
...
def test_a1_internal_action_runs_without_a_confirmation(owner: UserId, now: datetime) -> None:
    ...
    decision = require_execution_authorization(action, _allow(action, now), None, now)
...
def test_a3_needs_explicit_confirmation_not_a_standing_rule(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, now), standing, now)
...
def test_a3_without_an_audit_reference_is_denied(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, now), unaudited, now)
...
def test_an_authorization_for_another_action_is_rejected(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, now), other, now)
...
def test_an_expired_authorization_asks_again(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, later), authorization, later)
...
def test_changed_material_terms_invalidate_the_authorization(owner: UserId, now: datetime) -> None:
    ...
    assert _policy().authorize(agreed, _allow(agreed, now), authorization, now).is_permitted

    decision = _policy().authorize(repriced, _allow(repriced, now), authorization, now)
...
def test_incidental_parameters_do_not_revoke_consent(owner: UserId, now: datetime) -> None:
    ...
    assert _policy().authorize(retried, _allow(retried, now), authorization, now).is_permitted
...
def test_a_guardian_verdict_about_another_action_is_not_an_assessment(
    ...
    action = _action(owner, now, ActionDomain.PURCHASE, PermissionLevel.A3)
    authorization = _authorization(action, now, PermissionLevel.A3)
    elsewhere = DEFAULT_GUARDIAN_AUTHORITY.allow(
        action_id=ActionId("act_unrelated"), fingerprint="other", at=now
    )

    decision = _policy().authorize(action, elsewhere, authorization, now)
...
def test_consent_captured_under_a_block_never_becomes_valid(owner: UserId, now: datetime) -> None:
    ...
    decision = _policy().authorize(action, _allow(action, now), tainted, now)
...
def test_an_authorization_records_what_was_authorized(owner: UserId, now: datetime) -> None:
    ...
    assert not authorization.is_revoked


def test_an_isolated_authority_does_not_vouch_for_another_authoritys_verdict(
    owner: UserId, now: datetime
) -> None:
    """Holding the authority is the boundary: a verdict from elsewhere is not one."""
    action = _action(owner, now, ActionDomain.PURCHASE, PermissionLevel.A3)
    stranger = GuardianAuthority()
    verdict = stranger.allow(
        action_id=action.action_id, fingerprint=action.terms_fingerprint, at=now
    )

    decision = _policy().authorize(
        action, verdict, _authorization(action, now, PermissionLevel.A3), now
    )
